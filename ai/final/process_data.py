...
def emg_features(emg_segment, fs):
    # Nettoyage simple et rectification
    try:
    ...
    # Fréquence médiane: clamp de la bande à la fréquence de Nyquist
    nyquist = fs / 2.0
...
def resp_features(resp_segment, fs):
    ...
    try:
        rsp_signals, rsp_info = nk.rsp_process(resp_cleaned, sampling_rate=fs)
...
def derive_resp_from_ecg(ecg_signal, fs):
    """Construit une respiration dérivée de l'ECG (EDR) via NeuroKit.
    Gère différents formats de retour possibles de nk.rsp_ecg.
    Retourne un np.ndarray ou None en cas d'échec.
    """
    try:
        edr = nk.rsp_ecg(ecg_signal, sampling_rate=fs)
        # edr peut être array-like, dict, tuple
        if isinstance(edr, (list, np.ndarray, pd.Series)):
            return np.asarray(edr)
        if isinstance(edr, dict):
            for k in ["RSP", "EDR", "resp", "RSP_Signal", "ECG_EDR"]:
                if k in edr:
                    return np.asarray(edr[k])
            # fallback: premier élément
            try:
                return np.asarray(next(iter(edr.values())))
            except Exception:
                return None
        if isinstance(edr, tuple) and len(edr) > 0:
            return np.asarray(edr[0])
        # dernier recours
        return np.asarray(edr)
    except Exception:
        return None
...
def derive_resp_from_emg(emg_signal, fs):
    """Construit une respiration à partir d'un EMG respiratoire (CH3 typiquement).
...
def basic_stats(x):
    """Statistiques simples sur une fenêtre: mean, std, min, max, median, rms."""
    x = np.asarray(x)
    if x.size == 0:
        return {
            "mean": np.nan,
...
def process_file(file_path, window_s=10, overlap=0.5, label=None):
    """
    Version simple basée sur votre code initial:
    - charge le fichier OpenSignals
    - nettoie basiquement ECG/EMG/Resp
    ...
    # Mapping confirmé: CH1=ECG, CH2=EMG (membre), CH3=EMG respiratoire
    ecg = data.get('CH1')
    ...
    # Respiration dérivée de l'EMG respiratoire (CH3)
...
def build_dataset_from_folder(data_folder, window_s=20, overlap=0.5, label_map=None):
    ...
    Attendu (exemples):
      - start_user0.txt  (marche ~3 min)
      - sport_user0.txt  (course ~5 min)
      - recovery_user0.txt (marche ~2 min post-effort)
    ...
    for name in files:
        ...
        if 'start' in lname:
            activity = 'start'
        elif 'sport' in lname or 'run' in lname:
            activity = 'sport'
        elif 'recovery' in lname or 'rest' in lname:
            activity = 'recovery'
        # Extraire user id si présent
        m = re.search(r'user(\d+)', lname)
        user_id = int(m.group(1)) if m else None

        df = process_file(fpath, window_s=window_s, overlap=overlap, label=None)
        if df.empty:
            continue
...
if __name__ == "__main__":
    # Chemin relatif depuis ce fichier vers le dossier data (portable)
